...
public class CiudadOperaciones {
   ...
   public int agregar(Ciudad ciudad) {  
        int r=0;
        String sql="insert into ciudad(nombreCiudad, poblacionCiudad, codigoPais, distrito)values(?,?,?,?)";
        try{
    ...
    public int modificar(Ciudad ciudad) {  
        int r=0;
        String sql = "UPDATE ciudad SET idCiudad = ?, nombreCiudad = ?, poblacionCiudad = ?, distritoCiudad = ? WHERE codigoPais = ?";
            try{
                con = conectar.getConnection();
                ps = con.prepareStatement(sql);
